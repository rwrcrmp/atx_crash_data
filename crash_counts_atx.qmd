...
A major interstate cuts through the middle of downtown Austin, Texas. I-35 holds a potent historical significance for the city and radiates at the center of a current debate about its future. Impacts to the local community are substantial and multi-faceted, so any attempt to measure the highway’s effect on the surrounding population implies a choice of focus. In this blog, the dimension I examine is fatal vehicle collisions. This metric focuses attention because it’s more relatable than a distributed impact like pollution or economic development. After peaking during the pandemic, traffic deaths have marginally declined. The Department of Transportation estimates that [9,330 people died](https://crashstats.nhtsa.dot.gov/Api/Public/ViewPublication/813482) as a result of vehicle collisions in the first quarter of 2023. I intend to analyze other dimensions of community impact and perform some statistical work, but first I will simply count events in a defined area.

The nature of transportation data means that I get the opportunity to refresh some GIS skills. Even after making many maps before, I gained some knowledge around troubleshooting GIS irregularities and got to practice design. It’s also important to keep in mind that transportation data is inherently behavioral information. Counting vehicle collisions is a measure of stationary incidents, but human travel happens over non-uniform spaces at different speeds and times. This idea will become more important later when build a statistical model, but for now we can just "flatten the world" and take a raw count. I’ve started initial research into distance-decay models and how to create one in R for myself. The deliverable for this current project phase is the interactive map below. I’m still learning `leaflet` and iterating on the final design.
...
atx_crash_pts %>% 
  ...
  ggplot(aes(year, crash_counts)) +
    ...
    geom_col(aes(fill = crash_counts)) +
...
To determine which hexes contain the IH-35 roadway, I will use `st_intersects` which performs as its name indicates. `st_intersects` combined with `if_else` gives me a way to construct a logical variable concerning IH-35's presence within a given hex.

<center> __IH-35 Hexes in Turquoise__ </center>
```{r IH_35 hexes}
...
# apply T/F to color palette
names(custom_palette) <- levels(travis_grid$IH_35_hex)

travis_grid %>% 
  ggplot() +
    geom_sf(aes(fill = IH_35_hex)) +
    scale_fill_manual(values = custom_palette) +
    theme_void() +
    theme(legend.position = "none")
```

Although this hex classification isn't necessary to produce the end product for this stage, it points towards future analysis, and it's easy enough to include here. With the study area geometrically parceled, we can locate crash points within hexes, then group and count by hex.
...
Thank you for reading and following along the development process. I will be coming back to make revisions and clean up some of my code. The Github repo is a bit of a mess right now because this project involved a lot of trial and error to get working properly. I know that the interactive maps generated in the blog don't exactly match the results of the full screen version that I generated in a separate script with the same code. I'll be rewriting most of this script anyhow in the next stages, so for now, I'm happy with this as a methodological foundation and proof of concept.

In the future, I want to explore other variables in the CoA data and pull in traffic statistics from TxDOT and possibly other administrative datasets. My next major goal is to design a [distance-decay model](http://wiki.gis.com/wiki/index.php/Distance_decay), then I want to see how I might incorporate it into a machine learning model. So, basically I need to do some reading. This inital stage was another great opportunity for me to practice GIS while incrementally improving my web publishing skills, and I am excited to keep learning.
